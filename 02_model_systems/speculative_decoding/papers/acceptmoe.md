...
- 当前文档版本：`1.5.0`
- 当前修订 ID：`rev-acceptmoe-method-architecture-20260902`
- 当前修订时间：`2026-09-02T14:30:00+08:00`
...
| `rev-acceptmoe-zh-all-sections-20260902` | `1.3.0` | `2026-09-02T12:00:00+08:00` | `content-update` | `rev-acceptmoe-zh-method-flow-20260902` / `1.2.0` | 全篇术语审计，将非必要英文黑话改为中文并补充上下文解释 | 用户反馈；readability audit | minor |
| `rev-acceptmoe-accuracy-tradeoff-20260902` | `1.4.0` | `2026-09-02T14:00:00+08:00` | `correction` | `rev-acceptmoe-zh-method-flow-20260902` / `1.2.0` | 核对动态专家裁剪的精度代价，修正最坏单组下降数值 | AcceptMoE Introduction、§5.3、Table 1 | minor |
| `rev-acceptmoe-method-architecture-20260902` | `1.5.0` | `2026-09-02T14:30:00+08:00` | `content-update` | `rev-acceptmoe-accuracy-tradeoff-20260902` / `1.4.0` | 按新交付要求补充 4.3 的通俗算法流程、状态变化、近似边界与证据强度 | 用户反馈；AcceptMoE §3 | minor |
...
这一节把前面的公式串成一次实际验证过程。AcceptMoE 不是简单地“删掉几个专家”：它先估计哪些树节点更可能被提交，再据此估计本验证块真正需要哪些专家；接着保留根节点的原始 top-k 专家作为安全锚点，用需求分布的分散程度决定集合大小；如果专家卸载到主机内存，还要结合当前显存缓存继续删除低需求的未驻留专家。最后，所有 token 都在这个受限集合内重新做 top-k 路由。这样做的直接结果是访问的专家更少、权重搬运更少，但代价是部分 token 不再走目标模型原本选择的专家，因此会改变 verifier 的输出分布。

**提交概率加权：为什么不能把每个树节点一视同仁？**  
验证树中的兄弟节点最多只有一条路径会被接受，越深的节点也越不容易进入最终前缀。如果把每个节点的 router 分数直接相加，就会把最终一定会被丢弃的分支也算成同等重要的专家需求。AcceptMoE 用离线统计的 commitment probability 给位置加权，让更可能出现在最终输出中的节点贡献更大。论文在相同专家预算下报告平均准确率比 MoE-Spec 高 2.45 个百分点，说明这一步确实改善了“有限集合应该保留谁”的选择；但它依赖离线位置概率，未验证分布变化、任务变化或树形变化时概率是否仍可靠。

**根节点锚点：为什么不能完全按需求排序？**  
根节点是确定会参与验证的 token，它的自然 top-k 专家是最直接的高置信需求。AcceptMoE 强制把这些专家放进集合 $S$，再从剩余专家中按 utility 选择。这个锚点牺牲了一点可压缩空间，换来首 token 路由不被完全改写；论文没有单独移除锚点的消融，所以这是合理的保护性设计，而不是已被独立证明的最优策略。

**有效秩自适应大小：为什么每个 block 不用同一个固定 B？**  
不同模型、任务和验证块的需求分布可能很集中，也可能分散。固定保留 $B$ 个专家会出现两种浪费：需求集中时加载了很多用不到的专家，需求分散时又可能删得太狠。有效秩把需求熵转换成集合大小：越集中，保留集合越小；越分散，集合越大。论文在 5 组预算 sweep 中发现自适应点平均比各自最佳固定点低 0.97 个百分点、最坏低 1.83 个百分点。这说明它能避免逐任务调参，但不能说明有效秩在所有预算和 workload 上都是全局最优。

**驻留感知剪枝：为什么集合大小还不够？**  
在专家卸载场景，两个同样大小的集合可能有完全不同的成本：一个集合大部分专家已经在显存中，另一个集合大部分专家都要从主机加载。AcceptMoE 读取当前 LRU resident set，只从未驻留专家中优先删除低 utility 成员，并用 rerouting budget 限制额外改派的 token 数量。这样优化的是实际搬运和缓存驱逐，而不只是集合 cardinality。和关闭该剪枝的同一选择器相比，论文测到权重流量下降 38.6%–48.6%、吞吐提高 4.6%–15.1%，但平均准确率下降 0.27 个百分点，单组最坏下降 1.83 个百分点；这正是“减少开销换取有限精度风险”的直接证据。

**受限路由：为什么这是近似优化而不是无损优化？**  
得到 $S$ 或 $S'$ 后，论文把集合外专家的 router logit 设为 $-infty$，再在集合内选择 top-k。被删专家收到的 token 会被改派给保留专家，后续计算仍能完成，但此时 verifier 已不再执行目标模型的自然路由。因此 AcceptMoE 不是只做缓存预取，也不是严格保持目标分布的 speculative decoding。论文明确承认这一点，并只用任务准确率近似质量保持；尚未报告 token-level KL、perplexity、最坏任务或长尾领域结果。

**一次运行的边界。**  
算法选择的是“本验证块、某一 MoE 层”的临时允许集合，不是永久删除模型专家；专家权重仍保存在 GPU 或 pinned host memory 中。显存缓存不足时，保留但未驻留的专家仍可能按需加载，集合超过显存槽位时还会分波执行。因此“集合更小”不等于“传输为零”，最终负载仍取决于 cache residency、assignment 数、权重精度和 backend 调度。

下面的矩阵只做快速索引，详细解释以本节正文和 4.2 的公式卡为准：
...
| 驻留感知剪枝   | 传输量下降 38.6%–48.6%，吞吐提高 4.6%–15.1%；平均准确率变化 -0.27 个百分点 | Figure 5c–d、§5.3 | 同一选择器开关剪枝；单组变化 -1.83 到 +1.22 个百分点 | 直接支持但有精度代价 |
| 准确率边界    | 12 对平均低 0.27 个百分点，最坏单组下降 1.83 个百分点 | Table 1、§5.3 | 只看任务分数，未测 KL 或长尾质量  | 直接但范围窄         |

### 5.1 主结果

12 个模型—任务组合（Qwen3-Instruct/Coder、GPT-OSS-120B × GSM8K/MATH500/HumanEval/MBPP）中，AcceptMoE 平均准确率 90.63%，标准投机解码 90.90%，相差 -0.27 个百分点；各组合变化范围为 -1.83 到 +1.22 个百分点，因此最坏单组是下降 1.83 个百分点，而不是 1.22 个百分点。论文还明确说明，限制 target-expert eligibility 会改变模型分布，所以 AcceptMoE 不是严格 lossless SD。全专家驻留于 RTX PRO 6000 Blackwell、SGLang 0.5.12.post1、单请求下平均吞吐为 1.290 倍，范围 1.217–1.339 倍，平均接受长度均约 4.39，因此收益主要来自专家计算和访存减少，而不是接受长度增加。RTX 5090、每层 48 个显存槽位的专家卸载下平均为 2.06 倍；每个输出 token 的主机到显存传输量下降 73.6%–77.1%，缓存命中率提高 10.8–14.2 个百分点（Table 2）。
