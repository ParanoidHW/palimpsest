...
它的做法分两步：先比较同一步在“正常看图”和“视觉特征清零”条件下有多容易生成，据此离线构造短链；再把短链作为优选、原长链作为劣选，先监督微调，再做非对称偏好优化。v1 使用 860 个训练样本，实验直接支持输出显著缩短、四项基准准确率大体保持以及所测环境下时延下降；“抑制幻觉”“保证视觉依赖”和“有图梯度严格更大”仍缺少直接证据。

## 修订信息

- 当前文档版本：`1.0.2`
- 当前修订 ID：`rev-mcpo-v1-problem-scenario-20260917`
- 当前修订时间：`2026-09-17T14:20:23+08:00`
...
- 渲染结果：通过。修订 1.0.2 由 Pandoc 生成嵌入资产与 MathML 的独立 HTML，未产生数学转换警告；Chromium 输出 25 页 PDF。逐页检查首屏、§1.1 与 §2.1（第 1、6--7 页）以及 §4.4（第 13--16 页）：新增摘要翻译与六步具体场景完整显示，式 (1)--(13) 均正常显示，未见源码泄漏、编号重复、裁切、内容重叠或横向溢出。
