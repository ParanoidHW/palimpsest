...
> - 相关 Paper：[Linear Transformer](../papers/linear-transformer.md) · [RetNet](../papers/retnet.md) · [Mamba](../papers/mamba.md) · [Kimi K3](../papers/kimi-k3.md)
...
本版是机制与系统导航；当前已有 Linear Transformer、RetNet、Mamba、Kimi K3 四个相关 canonical Paper 链接，其余方法不冒充已验收证据。后续只有在 PDF/source、两类原论文视觉、代码 commit 与 schema/semantic checks 全部通过后，才能把对应条目升级为 canonical Paper，并同步覆盖矩阵。
