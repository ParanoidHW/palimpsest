...
> - 证据资产：[`../assets/papers/selfjudge/`](../assets/papers/selfjudge/)
> - 相关文档：[Figure inventory](../evidence/figure-inventory.md#selfjudge)
...
- 论文：[arXiv:2510.02329v2](https://arxiv.org/abs/2510.02329v2)，19 页；LaTeX source 与原始矢量图可解析。
- 开源代码：未发现官方仓库；源包仅含论文材料。
- OpenReview：未发现 SelfJudge 论坛，查询记录见 openreview_reviews.md。
- 提取文本：extracted_text/paper.txt。
- 图表与最终父级重裁 bbox：[正式 Figure inventory](../evidence/figure-inventory.md#selfjudge)。
...
- 第一作者（首位列名）：Kanghoon Yoon → KAIST。
- 共同第一作者（仅含论文明确标注者）：论文未显式标注。
- 通讯作者/通讯联系人（仅含论文明确标注者）：论文未显式标注。
- 其他作者涉及的机构（去重列举，不作逐作者映射）：KAIST；NAVER Cloud。
- 对应依据：论文 PDF 标题页、作者机构编号与角色脚注（核验日期：2026-07-30）。
- 边界说明：论文将 NAVER 实习与部分作者当前任职另列为脚注；未把未编号的经历说明改写为正式作者机构映射。
...
![Figure 2: SelfJudge 训练数据生成](../assets/papers/selfjudge/fig2-training-data-generation.png)
...
![Figure 1: GSM8K/MMLU 效率与质量](../assets/papers/selfjudge/fig1-domain-comparison.png)
...
![Figure 3: accepted length–accuracy trade-off](../assets/papers/selfjudge/fig3-speed-performance.png)

![Figure 4: suffix window sensitivity](../assets/papers/selfjudge/fig4-suffix-window.png)
...
- 论文 LaTeX source 只能核对公式、实验和硬件；无训练、data pipeline、verifier class 或 vLLM patch。

| 机制 | 本地证据 | commit | 判断 |
|---|---|---|---|
| semantic score | 论文 source §3.3.1 | 无 | 论文级，无法验证实现 |
| logistic verifier | 论文 source §3.4 | 无 | code unavailable |
| two-stage serving | 论文 source §4.5 | 无 | runtime 可读，实现未验证 |
