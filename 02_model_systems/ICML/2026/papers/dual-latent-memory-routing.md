...
- 当前文档版本：`1.1.0`
- 当前修订 ID：`rev-dlmr-problem-solution-20260725`
- 当前修订时间：`2026-07-25T10:05:32+08:00`
- 替代版本：`rev-source-recovery-20260724` / `1.0.1` / manifest `7706f839b1418a54fe51e366f454636e630d3f58c853ff2530ea18ccbbc7a16b`
...
作者摘要明确把出发点放在长程视觉语言推理：随着生成序列增长，模型既要持续引用早期图像证据，又要保留已经形成的中间结论与约束；把这两类信息都留在单一增长上下文中，会出现遗忘和冗余解码。该痛点发生在长回答的推理阶段，而不是视觉编码精度或基础模型预训练阶段。由于正文不可得，目前只能确认这是作者摘要陈述，无法核对其发生频率和基线诊断。

### 1.1.2 现有方案为何不够

摘要给出的失败模式是早期视觉 grounding 和中间 reasoning state 随上下文增长而丢失。作者隐含的根因是两类信息的生命周期与调用需求不同，却共享同一上下文表示和复用方式；本文据此重建这一解释，正文是否给出更严格的因果分析仍 `unavailable`。也无法确认普通长上下文、KV 压缩、检索记忆或显式 scratchpad 为何不足。

### 1.1.3 计划解决的问题与成功标准

- 核心问题：如何在冻结 base MLLM 的条件下，分别保存并按需复用视觉证据与推理状态。
- 约束：新增可训练参数应较少，长生成不能靠无界扩大上下文解决。
- 摘要级成功标准：一般与推理 benchmark 提升、路由呈状态依赖分工、冗余 decoding 减少。
- 证据边界：摘要没有指标定义、绝对数值、计算预算或 matched baseline，成功标准尚不能量化验收。

### 1.1.4 核心方案如何解决并优化问题

| 原始问题/失败模式 | 根因或约束 | 对应设计 | 改变的变量/系统行为 | 作用机制 | 预期优化 | 证据与判断 |
|---|---|---|---|---|---|---|
| 早期视觉证据遗失 | 视觉证据与增长文本混存 | visual latent memory | 视觉信息的存储与复用路径 | 压缩并独立保留 image evidence | grounding、长程一致性 | 摘要 author-stated；无正文实验，unverified |
| 中间结论和约束遗失 | reasoning state 缺少独立生命周期 | reasoning latent memory | 中间状态的持久化方式 | 独立跟踪并复用推理状态 | reasoning benchmark、长回答一致性 | 摘要 author-stated；unverified |
| 不同阶段需求不同 | 固定复用策略不能适应状态变化 | dynamic router | 每步选择的 memory 类型与复用量 | 按当前推理状态分配两类记忆 | token efficiency、减少冗余 decoding | 仅有摘要声明，unverified |
| 不希望重训基础模型 | 参数和训练预算约束 | frozen base + three-stage training | 可训练参数范围与训练顺序 | 分阶段构造记忆并学习选择性路由 | 参数效率、训练稳定性 | 阶段细节和参数量不可得，unverified |

### 1.1.5 完整因果链与证据闭环

当前可重建的链条是：长程视觉语言生成需要持续调用视觉证据和中间约束 → 单一增长上下文出现遗忘与冗余复用 → 将两类状态拆成 visual/reasoning latent memory → router 按推理状态选择复用对象和数量 → 预期改善 grounding、推理一致性与 token efficiency。前半段和设计名称由作者摘要直接支持；“根因是生命周期错配”属于本文推断；正文算法、改变变量的具体定义、benchmark 数值与因果消融全部缺失。因此该闭环目前只到“可解释方案假设”，尚未到“证据验证”。
