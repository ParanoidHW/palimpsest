...
> - 证据资产：[`../assets/papers/xdlm/`](../assets/papers/xdlm/)
> - 相关文档：[Figure inventory](../evidence/figure-inventory.md#xdlm)
...
- 主论文：[arXiv:2602.01362v1](https://arxiv.org/abs/2602.01362v1)，32 页；核验 PDF SHA-256 `2f2c23227f5d2202831a4a20dcb5364a708c044d591a463ad2d92c52fc1e0924`。
- 视觉证据：[正式 Figure inventory](../evidence/figure-inventory.md#xdlm)，3 个 PDF crop。
- 代码：[MzeroMiko/XDLM at `66c34ac5`](https://github.com/MzeroMiko/XDLM/tree/66c34ac5a3945d61e0e398f302bf751b5fadfa24)。下文代码路径均相对此 commit。
...
作者从离散扩散模型中“理解能力与少步生成能力难以同时最优”的经验冲突出发。Masked diffusion（MDLM）以 absorbing mask 逐步显露语义，zero-shot likelihood 和理解指标较强，但少步采样时上下文修正能力有限；uniform diffusion（UDLM）允许 token-to-token refinement，少步生成更强，却在理解、多步指标和大词表计算上付出代价。现有方法把两者视为不同 forward process，模型选择等同于提前固定 trade-off。

### 现有方案为何不够

单一 absorbing 或 uniform kernel 只覆盖权衡端点，无法连续调节随机 token refinement 与 mask semantic construction。已有插值方法若使用 time-varying transition，又需反复构造或处理大词表转移矩阵。根因有两层：建模层缺少可解释的统一噪声族，系统层的显式 $N\times N$ posterior/KL 在大词表上不可扩展。只做数学插值若不重写后验计算，仍无法得到可用吞吐和显存。

### 计划解决的问题与成功标准

- 核心问题：用一个可调、固定的噪声核统一 MDLM/UDLM，并使训练和采样在大词表上可计算。
- 约束：端点应解析退化为既有方法；中间混合系数 $k$ 可控制理解—生成权衡；后验不能显式物化 $N\times N$ 矩阵。
- 成功标准：不同 $k$ 在 PPL/FID/任务准确率上形成可解释 Pareto；scalar 实现改善 throughput/peak memory；端点等价有理论证明。
- 边界：最佳 $k$ 会随数据集、训练时长、采样步数和 CFG 改变；8B 结果还混合继续预训练与 checkpoint 初始化。

### 核心方案如何解决并优化问题

| 原始问题/失败模式 | 根因或约束 | 对应设计 | 改变的变量/系统行为 | 作用机制 | 预期优化 | 证据与判断 |
|---|---|---|---|---|---|---|
| MDLM/UDLM 各占一个权衡端点 | forward noise family 不统一 | uniform+mask stationary kernel | mask 与随机 token 噪声比例 $k$ | 连续插值 semantic construction 与 token refinement | PPL—generation Pareto | Eq. 7/16–19、$k$ sweep；supported |
| 时变核实现复杂 | 时间依赖进入完整转移矩阵 | stationary $K$ | 时间变化只保留在 $\alpha,\beta$ | 固定核简化 posterior 结构 | 可解释性、实现效率 | 理论直接支持；相对 GIDD 的独立归因有限 |
| 显式后验随词表平方增长 | $N\times N$ 转移/求和 | scalar posterior/KL | 计算变为 token probability、gather/scatter | 利用低秩 uniform 与 mask point mass | throughput、peak memory | Appendix K/Table 17、代码；supported as bundle |
| 连续时间小步长数值不稳 | $s\to t$ 时 log ratio 消减 | limiting loss | 使用极限形式替代不稳定项 | 避免灾难性消减 | 训练稳定性 | Lemma 3.5/代码；缺 approximation sweep |

### 完整因果链与证据闭环

MDLM 理解强而少步生成弱、UDLM 方向相反 → 根因是 absorbing mask 与 uniform replacement 提供不同的 refinement 能力，且既有统一方法的矩阵计算昂贵 → XDLM 用 $K=\frac{k}{N}J+\mu M$ 的 stationary kernel 连续连接两端，并利用其低秩/点质量结构把 posterior 和 KL 展开为标量运算 → 改变的是前向噪声构成、反向可修正路径以及大词表计算形态 → Figure 1 与多组 $k$-sweep 支持预算相关的质量权衡，Table 17 和代码支持 scalar 路径的内存/吞吐收益。尚未闭合的是：Table 17 同时改变数学与实现，8B 结果绑定 600-step 继续预训练，且 $k=0.1$ 并非跨任务常数；因此论文证明的是“可调统一框架和可扩展实现”，不是一个全局占优的固定 kernel。
...
| $Q_{t\mid s}$ | 从 $s$ 到 $t$ 的 row-stochastic 转移矩阵。 | author-defined | forward process | $N\times N$ | Eq. (1)、(5) | 代码不显式构造矩阵。 |
...
| $\alpha_t,\beta_t$ | signal/noise rate，$\beta_t=1-\alpha_t$。 | author-defined | schedule | $[0,1]$ | Eq. (5)、(10) | 代码由 `noise(t)` 返回。 |
...
|---|---|---|---|---|---|
| stationary $K$ | author-stated，§3.1 Eq. (5)–(7) | GIDD 的时变核需重复构造复杂转移矩阵 | 核结构固定，把时间依赖压到 $\alpha,\beta$，并保留统一极限 | 时变核表达力更高；固定核限制噪声 family | Eq. (6)–(7) 理论推导；Fig. 1 与 $k$ sweep 间接支持，未做 matched GIDD-only kernel ablation |
...
- `code/XDLM/algo.py:329-718`：MDLM 的 mask forward process、mask-only posterior；`718-875`：UDLM uniform replacement 和 `_compute_posterior`；`875-1027`：XDLM 的 $k_1$、`q_xt`、`_ancestral_update_core`。
...
按父契约，未生成 AI analysis diagram：当前安装的 OpenRouter ICU CLI 仅暴露 `generate`/`edit`，不支持技能要求的 required document-input path 文档输入路径；禁止用 prompt-only art 替代。因此论文原图 crop 是唯一内嵌视觉证据。
