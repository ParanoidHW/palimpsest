...
| Figure 12 | PDF p.21，`1654×2339` @ 200 DPI；bbox xywh `(150,565,740,840)` | Quadratic self-speculation：在 structured attention layout 中同时 draft 与 verify；绿色为通过 AR 验证的 draft，橙色为从最后匹配 token 出发、留待下一轮验证的 block。 | [asset](../assets/papers/nemotron-labs-diffusion/fig12-quadratic-self-speculation-caption.png)；支撑[Quadratic 机制](../papers/nemotron-labs-diffusion.md#quadratic-self-speculation-是-tree-like-verification-吗)。 | passed：单一 Figure 12、完整 caption、无 Figure 11/后续正文，`740×840` 原分辨率 QA（2026-07-31）。 |
