...
| --- | --- | --- | --- | --- |
| Figure 4 | PDF p.4，2550 x 3300；`(105,210,2325,735)` | ACT 架构：训练时 CVAE encoder 将动作序列和关节观测压成风格变量 z，测试时丢弃 encoder 并把 z 置为先验均值零；decoder 融合多视角图像、关节位置和 z，以 Transformer encoder/decoder 预测动作序列。 | [asset](../assets/papers/act/fig4-act-architecture-caption.png)；支撑 [核心机制](../papers/act.md#核心机制与贡献)。 | passed：单一 Fig. 4、完整 caption/标注、原分辨率可读。 |
| Figure 8 | PDF p.10，2550 x 3300；`(105,225,2335,770)` | chunk size 横轴与成功率纵轴表明两条基线均受益于 action chunk；Temporal Ensemble 改善 ACT 和 BC-ConvMLP、伤害 VINN；CVAE 对人类数据关键；5 Hz teleoperation 相比 50 Hz 使完成时间慢 62%。 | [asset](../assets/papers/act/fig8-ablation-caption.png)；支撑 [关键实验](../papers/act.md#关键实验与证据)。 | passed：四个 panel、坐标/legend/caption 完整。 |

## Diffusion Policy

| Object | Source | Caption（完整中文转述） | Crop / usage | QA |
| --- | --- | --- | --- | --- |
| Figure 2 | PDF p.3，2067 x 2924；`(170,184,1727,773)` | Diffusion Policy 总览：时刻 t 输入最近 To 步观测并输出 Ta 步动作；CNN 版本把观测 FiLM 条件注入每层卷积，Transformer 版本让每个 decoder block 以 cross-attention 接收观测；从高斯动作 A^K 经 K 次噪声预测得到去噪动作 A^0。 | [asset](../assets/papers/diffusion-policy/fig2_diffusion_policy_overview_caption.png)；支撑 [方法](../papers/diffusion-policy.md#方法与实现)。 | passed：单一 Figure 2、完整 caption、原 1727 x 773 可读。 |
| Table 4 | PDF p.8，2067 x 2924；`(174,184,835,926)` | 多阶段状态观测任务：PushBlock 的 px 是把 x 个 block 推入目标的频率，Kitchen 的 px 是交互 x 个以上对象的频率；Diffusion Policy 在较难的 BlockPush p2 与 Kitchen p4 上尤其领先。 | [asset](../assets/papers/diffusion-policy/table4_multistage_results_caption.png)；支撑 [关键实验](../papers/diffusion-policy.md#关键实验与证据)。 | passed：完整表、行列和 caption，原 835 x 926 可读。 |
