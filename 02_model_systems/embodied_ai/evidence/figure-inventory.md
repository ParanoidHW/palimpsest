...
| Object | Source | Caption（完整中文转述） | Crop / usage | QA |
| --- | --- | --- | --- | --- |
| Figure 2 | PDF p.3，`1700×2200` @ 200 DPI；`(130,140,1440,625)` | “VLFM constructs an occupancy map of the scene identifying frontiers of explored space as well as a value map of the likelihood of each region to lead toward the out-of-view target object. In a navigation episode, the robot first spins in a circle to initialize these maps and then begins executing frontier-based exploration by selecting waypoints from the current map frontier using the value map. Navigation to each waypoint is executed with a PointNav policy trained with Variable Experience Rollout (VER) [9]. The policy is also used to navigate to the target object once it is detected (‘goal navigation’).” | [asset](../assets/papers/vlfm/fig2-vlfm-architecture-caption.png)；支撑[研究方法](../papers/vlfm.md#4-研究方法)。 | passed：单一 Figure 2、完整 caption，contact-sheet + `1440×625` 原分辨率 QA。 |
| Table I | PDF p.5，`1700×2200` @ 200 DPI；`(855,140,710,555)` | “Zero-shot Object Navigation results on Gibson [6], HM3D [8], and MP3D [7] benchmarks. Our method outperforms previous zero-shot methods and performs competitively against methods directly trained on the Object Navigation task.” | [asset](../assets/papers/vlfm/table1-main-results-caption.png)；支撑[主结果](../papers/vlfm.md#51-主结果)。 | passed：单一 Table I、完整行列/caption，contact-sheet + `710×555` 原分辨率 QA。 |
| Table II | PDF p.6，`1700×2200` @ 200 DPI；`(140,140,710,330)` | “Comparisons of performance with different value update methods used with VLFM.” | [asset](../assets/papers/vlfm/table2-value-fusion-ablation-caption.png)；支撑[消融与技术点证据矩阵](../papers/vlfm.md#52-消融与技术点证据矩阵)。 | passed：单一 Table II、完整行列/caption，contact-sheet + `710×330` 原分辨率 QA。 |
...
| Object | Source | Caption（完整中文转述） | Crop / usage | QA |
| --- | --- | --- | --- | --- |
| Figure 2 | CVPR PDF p.3，`1700×2200` @ 200 DPI；`(140,175,1430,660)` | “Architecture Overview. Our model first patchifies the input images into tokens by DINO, and appends camera tokens for camera prediction. It then alternates between frame-wise and global self attention layers. A camera head makes the final prediction for camera extrinsics and intrinsics, and a DPT [57] head for any dense output.” | [asset](../assets/papers/vggt/fig2-architecture-caption.png)；支撑[研究方法](../papers/vggt.md#4-研究方法)。 | passed：重裁后单一 Figure 2、完整 caption，contact-sheet + `1430×660` 原分辨率 QA。 |
| Table 5 | CVPR PDF p.7，`1700×2200` @ 200 DPI；`(870,180,735,310)` | “Ablation Study for Transformer Backbone on ETH3D. We compare our alternating-attention architecture against two variants: one using only global self-attention and another employing cross-attention.” | [asset](../assets/papers/vggt/table5-backbone-ablation-caption.png)；支撑[技术点证据矩阵](../papers/vggt.md#52-技术点证据矩阵与消融机制证据)。 | passed：重裁后单一 Table 5、完整 caption，contact-sheet + `735×310` 原分辨率 QA。 |
| Table 6 | CVPR PDF p.7，`1700×2200` @ 200 DPI；`(870,490,735,275)` | “Ablation Study for Multi-task Learning, which shows that simultaneous training with camera, depth and track estimation yields the highest accuracy in point map estimation on ETH3D.” | [asset](../assets/papers/vggt/table6-multitask-ablation-caption.png)；支撑[技术点证据矩阵](../papers/vggt.md#52-技术点证据矩阵与消融机制证据)。 | passed：重裁后单一 Table 6、完整 caption，contact-sheet + `735×275` 原分辨率 QA。 |
...
| Object | Source | Caption（完整中文转述） | Crop / usage | QA |
| --- | --- | --- | --- | --- |
| Figure 2 | PDF p.4，`1530×1980` @ 180 DPI；`(168,146,1194,666)` | “WAM4D architecture and causal visibility pattern.” | [asset](../assets/papers/wam4d/fig2-wam4d-architecture-causal-visibility-caption.png)；支撑[研究方法](../papers/wam4d.md#4-研究方法)。 | passed：单一 Figure 2、完整 caption，contact-sheet + `1194×666` 原分辨率 QA。 |
| Table 7 | PDF p.12，`1530×1980` @ 180 DPI；`(168,152,1196,266)` | “Ablation of depth readout interface, register placement, and register visibility on the RoboTwin 10-task split with the depth head fixed. Selected quality metrics are reported.” | [asset](../assets/papers/wam4d/table7-register-interface-placement-visibility-caption.png)；支撑[技术 claim 证据矩阵](../papers/wam4d.md#53-技术-claim-证据矩阵)。 | passed：单一 Table 7、完整行列/caption，contact-sheet + `1196×266` 原分辨率 QA。 |
| Table 9 | PDF p.15，`1530×1980` @ 180 DPI；`(174,1097,1188,317)` | “Compute and latency comparison on a single A800 80GB GPU. Latency is reported as mean ± std in ms when available. Peak memory follows the VRAM measurement in Tab. 1.” | [asset](../assets/papers/wam4d/table9-deployment-latency-memory-caption.png)；支撑[Infra 需求分析](../papers/wam4d.md#8-infra-需求分析)。 | passed：单一 Table 9、完整行列/caption，contact-sheet + `1188×317` 原分辨率 QA。 |

## Xiaomi-Robotics-U0

| Object | Source | Caption（完整中文转述） | Crop / usage | QA |
| --- | --- | --- | --- | --- |
| Figure 3 | PDF p.5，`1700×2200` @ 200 DPI；bbox xyxy `(190,900,1510,1620)` | U0 总体结构：训练端统一为 next-token prediction；推理端支持标准 AR 与 FlashAR+。 | [asset](../assets/papers/xiaomi-robotics-u0/fig3-unified-architecture-caption.png)；支撑[研究问题](../papers/xiaomi-robotics-u0.md#1-研究问题与核心判断)。 | passed：单一 Figure 3、完整 caption、`1320×720` 原分辨率 QA。 |
| Figure 6 | PDF p.10，`1700×2200` @ 200 DPI；bbox xyxy `(190,1170,1510,1775)` | FlashAR+ 的二维反对角 step 调度与 prefix-conditioned step-causal attention mask。 | [asset](../assets/papers/xiaomi-robotics-u0/fig6-flashar-mask-caption.png)；支撑[反对角 step](../papers/xiaomi-robotics-u0.md#41-反对角-step)。 | passed：单一 Figure 6、完整 caption、无相邻正文，`1320×605` 原分辨率 QA。 |
| Figure 7 | PDF p.12，`1700×2200` @ 200 DPI；bbox xyxy `(190,235,1510,1290)` | AR 与 FlashAR+ 在 embodied transfer/scene generation 的质性比较，以及 GenEval、ImgEdit 与 1024² 生成耗时比较。 | [asset](../assets/papers/xiaomi-robotics-u0/fig7-flashar-efficiency-caption.png)；支撑[效率结果](../papers/xiaomi-robotics-u0.md#42-效率结果)。 | passed：单一 Figure 7、完整 caption、`1320×1055` 原分辨率 QA。 |
| Figure 17 | PDF p.22，`1700×2200` @ 200 DPI；bbox xyxy `(190,190,1510,1140)` | Original 与 U0-Aug 在三个任务和平均项上的 base/interference 任务进度及 drop from base。 | [asset](../assets/papers/xiaomi-robotics-u0/fig17-policy-robustness-caption.png)；支撑[下游策略鲁棒性](../papers/xiaomi-robotics-u0.md#53-下游策略鲁棒性)。 | passed：单一 Figure 17、完整 caption、`1320×950` 原分辨率 QA。 |
