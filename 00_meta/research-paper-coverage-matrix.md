...
>
> Obsidian 元数据：`90/90` 已完成深度精读的 canonical Paper 加入统一 YAML Properties（2026-09-18）。每篇均有 `paper`、collection、domain、`status/deep-review` 四项共同层级标签，以及各一项 `topic/*`、`method/*`；独立属性保留文档类型、canonical 状态和集合归属。其余短 Paper note 不计入该元数据审计分母。
...
| Transformers are SSMs: Generalized Models and Efficient Algorithms Through Structured State Space Duality（Mamba-2 / SSD；arXiv:2405.21060） | 深度精读；accepted-with-limitations；ICML 2024 | [mamba-2-structured-state-space-duality](../02_model_systems/llm_foundations/papers/mamba-2-structured-state-space-duality.md) |
| Gated Linear Attention Transformers with Hardware-Efficient Training（GLA；arXiv:2312.06635） | 深度精读；accepted-with-limitations；ICML 2024 | [gated-linear-attention](../02_model_systems/llm_foundations/papers/gated-linear-attention.md) |
| Parallelizing Linear Transformers with the Delta Rule over Sequence Length（DeltaNet；arXiv:2406.06484） | 深度精读；accepted-with-limitations；NeurIPS 2024 | [deltanet](../02_model_systems/llm_foundations/papers/deltanet.md) |
| Gated Delta Networks: Improving Mamba2 with Delta Rule（Gated DeltaNet；arXiv:2412.06464） | 深度精读；accepted-with-limitations；ICLR 2025；OpenReview 正文不可得 | [gated-deltanet](../02_model_systems/llm_foundations/papers/gated-deltanet.md) |
...
| Why Low-Precision Transformer Training Fails: An Analysis on Flash Attention（arXiv:2510.04212v4） | 深度精读；accepted-with-limitations；ICLR 2026；生产 fused kernel 泛化待验证 | [low-precision-flash-attention](../01_ai_infra/kernel/custom_attn/papers/low-precision-flash-attention.md) |
...
| EchoWM: Open and Enterable Omnimodal World Models（arXiv:2608.23189v1；别名：Echo-WM、EchoWM-Flash） | 深度精读；相对 6-DoF 相机意图、联合音视频、4 步因果 rollout 与有界缓存 | [echowm](../02_model_systems/multimodal_generation/papers/echowm.md) |
...
| Approximate Speculative Decoding（arXiv:2608.03447v3） | 深度精读；局部 regret、block cap、请求级账本与连续后缀复用；含代码与数值口径审计 | [approximate-speculative-decoding](../02_model_systems/speculative_decoding/papers/approximate-speculative-decoding.md) |
