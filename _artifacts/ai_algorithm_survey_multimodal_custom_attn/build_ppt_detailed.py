...
ROOT = Path('01_ai_infra/kernel/custom_attn')
OUT = ROOT / '多模态稀疏Attention与定制Mask_Kernel调研.pptx'
...
def add_title(slide, title, dark=False, subtitle='图文精读版 | 2026-07-10 | NVIDIA CUDA 重点'):
    add_text(slide, title, .55, .28, 11.9, .45, 27, WHITE if dark else INK, True, font='Aptos Display')
    add_text(slide, subtitle, .58, .88, 11.8, .22, 9, 'B7C8C4' if dark else GREY)

def add_footer(slide, number, source=None, dark=False):
    if source:
        add_text(slide, source, .58, 7.04, 11.6, .18, 8, 'AAB8B5' if dark else GREY)
    add_text(slide, f'{number:02d}', 12.42, 7.01, .35, .2, 10, 'AAB8B5' if dark else GREY, True, PP_ALIGN.RIGHT)
...
for i,(a,b,col) in enumerate([('理解', 'high-resolution VLM token selection', MINT),('统一', 'two-way stream lowering / special causal', GOLD),('生成', 'window, CSR, router, temporal reuse', CORAL)]):
    y=4.0+i*.72; add_box(s,8.82,y,3.65,.48,col,col,True); add_text(s,a,9.04,y+.12,.65,.18,13,INK,True); add_text(s,b,9.75,y+.11,2.5,.2,11,INK)
...
paper_slide(4,'理解侧：FlexAttention VLM 的高分辨率 token selection', ASSET/'2407_flexattention_vlm/fig2_hierarchical_vlm_selection_caption.png', '所有 high-resolution patch 与文本一起全注意力，分辨率升高时 decoder 成本失控。', '低分辨率/文本先给全局语义；attention map 选出高分辨率区域 token，再进 hierarchical self-attention。', 'selected indices -> gather high-res feature -> compact attention。具体 CUDA backend 本次未逐行核验，不能把方法名等同 PyTorch FlexAttention。', 'Paper Fig.2, arXiv:2407.20228; official UMass repository', MINT)
paper_slide(5,'统一模型：Cosmos 3 将混合 mask 降低为两个 varlen 调用', ASSET/'2606_cosmos3/two_way_attention_infra.png', 'reasoner 必须 causal 且不受 noisy generator 污染；generator 又必须读同样本 reasoner + 自身 token。', 'reasoner: causal self-attention；generator: bidirectional attention over [reasoner, generator]。按 sample 打包，避免跨样本 read。', '先 semantic lowering 为 causal/full 两次调用。论文材料报告相对 FlexAttention baseline 的 Nano training throughput +22%，Hopper FA3、GB200 NATTEN/CUTLASS。', 'Cosmos 3 local source: two-way attention infrastructure', GOLD)
paper_slide(6,'流式世界模型：Causal-rCM 的 special causal mask', ASSET/'2606_causal_rcm/fig3_causal_training_paradigms_caption.png', 'AR diffusion 的 TF 需要 clean history + noisy target；SF 又需 self-generated rollout 与 KV cache。普通三角 causal mask 不够。', '图中 TF / DF / SF 区分 clean、noisy、self-generated history；noisy block 只读允许的 clean history 与自身 block。', 'BlockPattern + AttnMaskSpec -> Flex BlockMask / range metadata；同一 mask 进入 primal 和 JVP Triton kernel。Magi backward 限制需单独核验。', 'Causal-rCM Fig.3; code commit ed3cb14', CORAL)
paper_slide(7,'长视频：LVSA 用 window + rotating anchors 保持稀疏预算', ASSET/'2605_lvsa/fig1_expanded_window_caption.png', '固定 window 会漏长程依赖；window 与 global anchor 重叠又浪费固定 attention budget。', 'expanded local window 与 periodic global frames 构成 A(t)=G union W(t)，每个 query frame 近似保持相同 attended set 大小。', 'frame-block CSR int32 indptr/indices；FlashInfer BlockSparseAttentionWrapper 跳过未列 tile。CPU planner 留 metadata 在 host，并非 GPU kernel 直接读 CPU RAM。', 'LVSA Fig.1; code commit 1ebcc92', MINT)
paper_slide(8,'学习式 block router：VMoBA 的 partition -> select -> varlen', ASSET/'2506_vmoba/fig2_vmoba_pipeline_caption.png', '一维均匀 MoBA block 与视频 temporal/spatial/3D 邻域不匹配；固定 top-k 也浪费异质 head 预算。', 'recurrent 1D/2D/3D partition，mean key 产生 block score；global/threshold selection 后仅在选中 blocks attention。', 'GPU gate + topk/threshold -> nonzero -> gather QKV -> cu_seqlens -> FlashAttention varlen。控制面为 gate/sort/pack/LSE，不传 CSR 或 pair-mask。', 'VMoBA Fig.2; code commit 48aaccd', GOLD)
paper_slide(9,'动态控制面：HASTE 的 mask reuse 与 head-wise calibration', ASSET/'2605_haste/fig4_tmr_ebc_framework_caption.png', 'Video DiT 要多 step denoise；逐 head、逐 step 重算 sparse mask 可能吃掉 attention 节省，统一 threshold 又不公平。', 'TMR 用 Q/K drift 判断每个 head 复用还是刷新 cached descriptor；EBC 用离线 error curve 分配 head-specific threshold。', '应缓存 sparse descriptor，而非 N x N mask。官方代码未取得，CSR/BlockMask/host-device placement 不可断言；机制与结果为 PDF-only。', 'HASTE Fig.4, arXiv:2605.14513', CORAL)
paper_slide(10,'Training-free 视频：Sparse VideoGen 的 spatial / temporal head dispatch', ASSET/'2502_sparse_videogen/fig4_svg_workflow_caption.png', 'Video DiT 3D full attention 成为主成本；直接移植 LLM mask 会丢 temporal dependency。', 'sampled rows 对 spatial / temporal / full attention 做 MSE 近似，按 head dispatch 到专用 pattern。', '关键不只是找 pattern：temporal slash 必须 layout transform 才能有 coalesced tile access。论文称 Triton/FlashInfer prototype，具体 metadata 本次未有源码核验。', 'Sparse VideoGen Fig.4, arXiv:2502.01776', MINT)
paper_slide(11,'Token Sparse Attention：保留 selector 灵活性，复用成熟 kernel', ASSET/'2602_token_sparse_attention/fig3_compress_attention_scatter_caption.png', 'block sparse 粒度高效但 token importance 随 layer/head 变化；永久 drop token 会妨碍后续层重新选择。', 'per-head select token subset；compress Q/K/V；在 compact tensors attention；scatter output back 后叠加 residual。', 'kernel 只见连续 compact QKV，可复用 FlashAttention；真实成本为 selector + gather/contiguous + scatter。实现细节为 PDF-only。', 'Token Sparse Attention Fig.3, arXiv:2602.03216', GOLD)
paper_slide(12,'长上下文桥接：MInference 的 pattern-aware index 与 kernel dispatch', ASSET/'2407_minference/fig3_sparse_patterns_caption.png', 'prefill attention 延迟主导；固定 top-k index 跨 prompt recall 显著下降。', '离线给 head 选 A-shape / vertical-slash / block-sparse family；在线建立具体 ranges/columns/blocks。', 'pattern-specific index 交给 PIT/Triton/FlashAttention 类 kernel。迁移到 video 必须改为双向时空 pattern，并处理每 step 的 planner 成本。', 'MInference Fig.3, NeurIPS 2024 Spotlight', CORAL)
paper_slide(13,'架构替代：FrameDiT 以 matrix attention 改变 temporal topology', ASSET/'2603_framedit/fig1_matrix_attention_architecture_caption.png', 'full 3D attention 表达强但昂贵；local factorized attention 便宜却错过大运动。', '以 frame matrix 为对象做 temporal Matrix Attention，Global-Local hybrid 保留局部路径，不再构造原 token-pair temporal mask。', '公开代码仍把 2D mask 转为 -10000 dense bias 并 broadcast；论文算法收益不自动等于 custom sparse kernel。', 'FrameDiT Fig.1, CVPR 2026 Findings; code commit 359bd12', MINT)
...
callout(s,'评测公式','Ttotal = Tselect + Tmetadata + TH2D/plan + Tpack + Tattn + Tunpack。只报 attention FLOPs 会遗漏控制面和数据搬运。',.95,5.55,11.4,.75,GOLD); add_footer(s,17)
...
add_text(s,'完整图文精读、PDF/source、代码 commit、图清单与 QA：_artifacts/ai_algorithm_survey_multimodal_custom_attn',.78,6.7,11.8,.22,10,'A9BFBA'); add_footer(s,18,dark=True)
