...
> 资料状态：arXiv:2603.12617v1 的 PDF、HTML、LaTeX source 与作者代码均可取得。PDF 为 27 页；source 中的版式明确显示 “Published as a workshop paper in Lifelong Agent @ ICLR 2026”，因此不能把任务清单中的 ICML 2026 candidate 标记解释为 ICML 接收。原图采用 PDF 200 DPI 渲染裁剪，原始矢量素材另存于 `source/unpacked/figs/`。

## 修订信息

- 当前文档版本：`1.0.0`
- 当前修订 ID：`rev-onlinespec-initial`
- 当前修订时间：`2026-07-17T09:30:00+08:00`
- 替代版本：无（initial）
...
- 论文：`paper.pdf`；摘要与版本页：`arxiv_page.html`、`arxiv_html.html`
- source：`source/source.tar` 与 `source/unpacked/`
- 代码：`https://github.com/ZinYY/OnlineSPEC`，commit `3a6cc69d1c839385fcdd5f82529c55300e503b4b`
- OpenReview：不适用；arXiv 页面和 source 未提供 OpenReview forum/review/decision/rebuttal。
- 提取文本：`extracted_text/full_text.clean.txt`、`extracted_text/pdftotext.txt`
- 图表：Figure 1/2/3、Table 1/2，完整 inventory 在 `figure_inventory.md`。
- AI 生成分析图：跳过；当前 ICU CLI 没有契约要求的 `responses-doc --input-file analysis.md` 能力。
...
| (A,a) | target、draft 的期望推理时间 | author-defined | 每轮 | 时间；(alpha=a/All1) | Theorem 1 | 未给硬件与绝对毫秒 |
...
| (mathrm{Reg}_T) | dynamic regret | author-defined | 1..T | 累计损失 | Eq. (2) | 理论量，实验没有直接估计 |
| (eta) | learning rate | author/code-defined | update | 无量纲超参 | §3、Table 3 | 单固定 η 在分布漂移下不稳 |
| (mathbf h_t), (delta_T) | optimism hint、hint 误差上界 | author-defined | 每轮/累计 | 向量、平方范数和 | Eq. (3)、Cor. 2 | 代码将上一梯度作为 hint，但未报告 (delta_T) |
| (N,arepsilon,p_t^i) | ensemble 基模型数、Hedge 灵敏度、权重 | author-defined | learner i, round t | (N=O(\log T)), simplex weights | §3.3 | 论文公式写出模型参数加权，代码需核对输出融合细节 |
...
![Figure 1](figures/crops/fig1_generation_refinement_caption.png)

Figure 1 展示 draft→parallel target verification→feedback loop；它支持框架直觉，不是速度数据。

![Table 1](figures/crops/table1_main_results_caption.png)
...
![Table 2](figures/crops/table2_reasoning_results_caption.png)
...
![Figure 3](figures/crops/fig3_tps_evolution_caption.png)
...
- 仓库：`code/OnlineSPEC`，commit `3a6cc69d1c839385fcdd5f82529c55300e503b4b`。
