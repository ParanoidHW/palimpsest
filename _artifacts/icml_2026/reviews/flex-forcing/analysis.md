...
- 标题、URL、候选 venue 信息来自任务包 `task_packet.yaml`；任务包备注称 v1 于 2026-07-03 发布，并要求核验 ICML 2026 状态。
- `paper.pdf` 仅 1.8 MiB；`pdfinfo` 与 `pdftotext` 均报告 `Couldn't find trailer dictionary`、`Couldn't read xref table`。无法读取正文、公式、表格、图注、附录或实验数字。
...
| 术语 | 定义 | 来源 | 歧义 |
|---|---|---|---|
| Flex-Forcing | 任务包给出的论文标题中的方法名 | `task_packet.yaml:title` | 论文正文不可读，无法确认具体算法含义 |
| autoregressive | 标题中的自回归生成范式 | `task_packet.yaml:title` | 未知其在论文中对应训练、采样还是混合模式 |
| bidirectional video diffusion | 标题中的双向视频扩散范式 | `task_packet.yaml:title` | 未知具体噪声调度、注意力掩码与条件定义 |
...
- OpenReview：任务包无 forum URL，且本次不进行网络恢复；`openreview_reviews.md` 记录不可用状态。
...
见 `figure_inventory.md`。因 PDF trailer/xref 损坏，无法生成 page PNG、单对象紧裁剪或 contact sheet；不存在可接受的机制图或结果/系统图。
