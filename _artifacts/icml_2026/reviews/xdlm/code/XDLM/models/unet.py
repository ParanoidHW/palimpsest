...
from .dit import LabelEmbedderx, EmbeddingLayer
...
class UNet(nn.Module):
    def __init__(self, config, vocab_size=None):
        ...
        if config.training.guidance:
            ...
            self.cond_map = LabelEmbedderx(
                self.num_classes + 1,  # +1 for no condition
    ...
    def forward(
        ...
        if cond is not None:
            if self.cond_map is None:
                ...
                """
                in training, we (may) falsely set labelemb to num_classes + 1 
                (should be num_classes as it'll automatically +1 in lebelemb)
                so now, reset nocond label.
                """
                label_size = self.cond_map.embedding_table.weight.shape[0]
                if label_size > self.num_classes + 1:
                    nocond_id = label_size - 1
                    cond = torch.where(cond == self.num_classes, nocond_id, cond)
                else:
                    assert label_size in [self.num_classes + 1, self.num_classes]

                temb = temb + self.cond_map(cond, self.training)
