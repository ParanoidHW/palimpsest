...
# copied from dit_llama.py
class LabelEmbedderx(nn.Module):
    ...
    def forward(self, labels, train, force_drop_ids=None):
        assert (
            labels.max() < self.embedding_table.weight.shape[0]
        ), f"labelemb failed for label {labels}"
...
class DDiTBlockCausal(nn.Module):
    ...
    def forward(self, x, rotary_cos_sin, is_causal=None, **kwargs):
        ...
        x = flash_attn.flash_attn_interface.flash_attn_varlen_qkvpacked_func(
            qkv, cu_seqlens, seq_len, 0.0, causal=is_causal
        )
...
class DDiTBlock(nn.Module):
    ...
    def forward(self, x, rotary_cos_sin, c=None, **kwargs):
...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, config, vocab_size: int):
        ...
        if config.training.guidance:
            class_dropout_prob = config.data.class_dropout_prob
            num_classes = config.data.num_classes
            self.y_embedder = LabelEmbedderx(num_classes, cond_dim, class_dropout_prob)
    ...
    def forward(self, x, sigma, cond=None, is_causal=None):
        x = self.vocab_embed(x)

        tmp_t_cond = None
        if self.sigma_map is not None:
            tmp_t_cond = self.sigma_map(sigma)

        tmp_y_cond = None
        if self.y_embedder is not None:
            tmp_y_cond = self.y_embedder(cond, self.training)

        if (tmp_t_cond is not None) and (tmp_y_cond is not None):
            t_cond = F.silu(tmp_t_cond + tmp_y_cond)
        elif tmp_t_cond is not None:
            t_cond = F.silu(tmp_t_cond)
        elif tmp_y_cond is not None:
            t_cond = F.silu(tmp_y_cond)
        else:
            t_cond = None
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            for i in range(len(self.blocks)):
                x = self.blocks[i](x, rotary_cos_sin, c=t_cond, is_causal=is_causal)
            x = self.output_layer(x, c=t_cond)

        if (is_causal is not None) and (not is_causal) and self.causal:
            x.__info__ = {"is_causal": False, "t_cond": False}
