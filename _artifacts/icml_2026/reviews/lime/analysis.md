...
- 论文：`paper.pdf`；来源 `https://arxiv.org/abs/2604.02338v1`，提交 2026-02-01，正文标注 preprint April 6, 2026。
- 源码/LaTeX：`source.tar` 为未完成 gzip partial，未使用；失败原因见限制。
- 开源代码：GitHub 搜索 API 找到 `https://github.com/vk032503/lime-lightweight-mixture-of-experts`，浅克隆未完成且无可验证 commit（网络/DNS 限制），因此代码结论均标为未验证。
- OpenReview：不可用；见 `openreview_reviews.md`。
- 提取文本：`extracted_text/extracted_text/full_text.clean.txt` 与 `extracted_text/paper.txt`。
- 图表：`figures/crops/fig1_architecture_caption.png`、`table2_results_caption.png`、`fig2_efficiency_caption.png`；完整 caption、页码、bbox 和 QA 见 `figure_inventory.md`。
- AI 生成分析示意图：跳过。委派合同指定当前 CLI 只支持 `generate`/`edit`，不支持必需的 `responses-doc --input-file analysis.md`，不能以 prompt-only 图替代。
...
跳过：父合同明确记录 CLI 不具备 `responses-doc --input-file analysis.md` 文档输入路径；不生成 prompt-only 图片。
...
![Figure 1 mechanism](figures/crops/fig1_architecture_caption.png)
...
![Table 2 results](figures/crops/table2_results_caption.png)
...
![Figure 2 efficiency](figures/crops/fig2_efficiency_caption.png)
...
公开 OpenReview forum、review、meta-review、decision、rebuttal 未获取（DNS 失败；见 `openreview_reviews.md`）。因此不存在可交叉核验的 reviewer claim，不应把“ICML 2026 candidate”视为接收结果。
