...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `rev-initial-20260716` | `1.0.0` | `2026-07-16T19:16:43+08:00` | `review_dual_latent` | `initial` | 无 | 无 | 建立官方摘要级 blocked 交付 | 精确 PDF 在受控恢复后仍不可得 | `analysis.md`、`figure_inventory.md`、`openreview_reviews.md` | ICML poster 63955；`recovery/recovery_log.md` | material：不能验证论文技术与实验结论 |

## 0. 资料与配图索引

- 官方论文页：<https://icml.cc/virtual/2026/poster/63955>
- 官方页面快照：`recovery/icml-poster-63955.html`
- PDF：不可得。
- LaTeX/source：不可得。
- 开源代码：未发现官方仓库。
- OpenReview：官方页无链接；API challenge 403，见 `openreview_reviews.md`。
- 恢复记录：`recovery/recovery_log.md`。
- 图表：0；见 `figure_inventory.md`。没有生成空 contact sheet。
- AI 生成分析示意图：跳过。父契约确认已安装 CLI 只有 `generate`/`edit`，不具备技能强制要求的 `responses-doc --input-file analysis.md` 文档输入路径。
...
官方页没有 OpenReview 链接或 forum ID；OpenReview v1/v2 exact-title API 均返回 challenge 403。reviews、meta-review、decision、rebuttal 和 discussion 均不可得，详见 `openreview_reviews.md`，因此不能进行评审交叉核验。
