...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `rev-latentlm-20260716-initial` | `1.0.0` | `2026-07-16T18:50:00+08:00` | `review_latentlm` | initial | 无 | 无 | 首次建立论文级证据分析、视觉 QA、代码/OpenReview/infra 边界 | 父任务 `icml2026-latentlm-004` | `analysis.md` 全文及同目录证据 | arXiv v1 PDF、LaTeX、Figure 2/7、任务契约 | material |

## 0. 资料与配图索引

- 论文：`paper.pdf`，SHA-256 `15f74eb68e4c8785d82139e160e84bb3b083b14a0256a7a9bb4735a374516ec0`
- LaTeX：`source/latex/main.tex`；下载流异常见“局限”，但正文与图像成功提取
- 开源代码：`code/unilm/LatentLM/`，commit `833df7e7832e5064a281131ee64a481afa8e5b95`；README 的 Setup/Usage 与 pretrained models 均为 “Coming soon”
- OpenReview：未发现可保存的公开评审；取证记录见 `openreview_reviews.md`
- 提取文本：`extracted_text/paper.txt`
- 图表：Figure 2 `figures/crops/fig2_latentlm_architecture_caption.png`；Figure 7 `figures/crops/fig7_inference_throughput_caption.png`；详见 `figure_inventory.md`
- AI 生成分析示意图：未生成。父契约确认本地 CLI 仅有 `generate/edit`，缺少强制的 `responses-doc --input-file analysis.md` 路径。
...
![Figure 2: LatentLM architecture](figures/crops/fig2_latentlm_architecture_caption.png)
...
![Figure 7: inference throughput](figures/crops/fig7_inference_throughput_caption.png)
...
未取得公开 OpenReview 页面或评审。arXiv v1 无 venue comment；OpenReview exact-title API 在本环境返回 403，网页搜索后端失败。故无法分析 reviews/meta-review/decision/rebuttal，详见 `openreview_reviews.md`。这尤其意味着不能把任务清单中的 ICML 2026 候选身份当成接收事实。
...
- 本地 snapshot：`code/unilm/LatentLM/`，commit `833df7e7832e5064a281131ee64a481afa8e5b95`。
