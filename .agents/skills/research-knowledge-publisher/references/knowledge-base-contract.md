...
Repository governance is authoritative. Read applicable `AGENTS.md` files and the research-knowledge organization specification before using this contract. This reference defines the workflow interface, not a competing directory policy.
...
- Keep raw search results, PDFs, source trees, page renders, extraction attempts, scripts, logs, and manifests in `_artifacts`.

## 5. Cross-Domain Example

For a video model with a native sparse-attention component:

- the full model report may be owned by `02_model_systems/multimodal_generation/papers/`;
- sparse-attention adoption evidence may be owned by `01_ai_infra/kernel/custom_attn/evidence/`;
- the sparse-attention Survey links to the model Paper and adoption Evidence;
...
8. No formal document references `_artifacts`, local absolute paths, page renders, or temporary files.
