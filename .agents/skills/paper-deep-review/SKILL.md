...
- Explain the motivating problem before introducing the paper's method names, module acronyms, or equations. Near the opening or in `### 2.1 出发点与背景痛点`, include at least one concrete end-to-end scenario that names the affected actor/object, input or operating context, what the prior/current workflow does, the observable failure, its practical consequence, and the behavior the paper intends to change. Prefer a paper-provided example; otherwise label the scenario as a reviewer reconstruction and keep every detail evidence-bounded. A generic statement such as “accuracy is low,” “reasoning is inefficient,” or “existing methods ignore heterogeneity” does not satisfy this gate.
...
- Treat plain-language terminology as a whole-delivery requirement. Audit every section, heading, table, figure caption, formula card, glossary entry, generated diagram label, and status field in `analysis.md`, not only the method section. Retain English only for necessary abbreviations, proper names, paper-defined symbols, or terms that are genuinely standard in the target research community; explain each retained term in Chinese at first use and prefer Chinese wording for ordinary concepts. Review-internal labels such as `confounded`, `plausible`, `frontier`, `proxy`, `telemetry`, `lifecycle`, `baseline`, `pipeline`, and `overhead` must be translated or explicitly explained when they appear. A delivery with unexplained nonessential jargon is incomplete even when all evidence and formulas are present.
- Treat rendered Markdown as a completion gate. Render the final `analysis.md` with the repository's available Markdown renderer (including Obsidian-compatible preview when available) and inspect the rendered output. Tables, formulas, image links, captions, headings, lists, and anchors must render without broken syntax, swallowed rows, literal markup, overflow, or accidental blank blocks. A source-level regex or schema pass does not substitute for rendered-output verification; record the renderer, command, and result in the checklist or validation artifact, and block completion when rendering cannot be run.
- Treat evidence placement as a completion gate. Every embedded paper Figure/Table must appear immediately before or after the claim, mechanism, experiment, or limitation it supports, with a nearby explanatory sentence. Inventory presence or filename matching alone is insufficient: verify section-level semantic proximity and reject figures placed only in a generic appendix/results block unless the inventory records a justified redundancy or cross-reference reason.
- Treat temporary markup as forbidden in frozen delivery. Before validation, scan `analysis.md`, figure captions, tables, generated diagrams, and metadata for HTML comments (`<!-- ... -->`), validator markers, TODO/pending placeholders, debug text, and process-only annotations. Remove them or classify the delivery as blocked; comments used during drafting must never be carried into the canonical Paper projection.
...
- Run the global-completeness gates defined in [references/completeness-contract.json](references/completeness-contract.json). The validator must reject missing or reordered template sections, shortened checklist item sets, one-way manifest declarations, process/canonical projection drift, or mismatches among formula blocks, rationale/failure tables, visual files, inventory rows, and Markdown references. Final delivery validation must use the default strict mode.
...
- Treat every extracted crop under `figures/crops/` as a delivery object. Before freeze, require a one-to-one mapping: each crop has exactly one inventory row, an actual Markdown image reference in `analysis.md`, a nearby claim/explanation, and a complete caption path; an extracted crop that is only present in the asset directory or contact sheet is an incomplete delivery. The validator must fail on any unused crop or inventory asset.
...
4. **Read with evidence discipline.**
   ...
   - Before naming the proposed method, walk through one representative case from input to failure and desired outcome. Make the reader able to point to the exact step that breaks and say what the method must preserve, remove, predict, route, accelerate, or otherwise change. State whether the case is quoted/paraphrased from the paper or reconstructed for explanation.
...
- The design-rationale matrix is a summary index, never the explanation itself. In `### 4.2 组件级设计动机与具体问题映射` (or the corresponding method subsection), write reader-facing plain-language prose for every core design item before/after the summary table. Each item must explain the concrete failure in an observable scenario, what the design changes, why that change could help, its trade-off, and which evidence verifies or fails to isolate it. A table-only method section, or prose that merely restates component names, must fail readability validation.
...
- Validate the rendered Markdown before freeze. Use an available CommonMark/Markdown renderer or Obsidian-compatible preview, inspect the rendered `analysis.md`, and record the renderer and result. Do not declare delivery complete from source parsing alone. If no renderer is available, record the exact blocker and keep the delivery `blocked`.
- Place every embedded Figure/Table adjacent to the prose claim it supports. For each object, record the supporting section/claim in `figure_inventory.md` and verify the actual Markdown order during rendered-output review. A figure that is merely present somewhere in the document does not satisfy this requirement.
- Remove all temporary HTML comments and process markers before freeze. The frozen review and canonical Paper must contain no `<!-- ... -->`, `TODO`, `FIXME`, `pending`, debug labels, or validator-only annotations unless the text is explicitly part of the paper evidence and explained as such.
...
4. Recompute all frozen artifact hashes in `deliverable_manifest.json`, rerun structural, semantic, and global-completeness validation, and freeze the final deliverable manifest. A `passed` validation must have an empty error list. In standalone promotion, the validator resolves `knowledge-promotion-plan.json` and checks the canonical Paper projection; `--canonical-paper` is reserved for isolated tests or an explicitly located projection.
...
- Render the final `analysis.md` with an available Markdown/CommonMark or Obsidian-compatible renderer and inspect the rendered result. Confirm every table, formula, image, caption, heading, list, and anchor renders correctly; source-level parsing alone is insufficient. Record renderer, command, and result, and keep completion blocked if rendering cannot be performed.
- Confirm every embedded Figure/Table is semantically adjacent to the prose claim or method/experiment explanation it supports. Check the actual rendered order, not only inventory filenames or section labels; require an explicit inventory reason for any intentional non-local placement.
- Scan the frozen review and canonical Paper for `<!-- ... -->`, `TODO`, `FIXME`, `pending`, debug text, validator markers, and process-only annotations. Any such residue must be removed or explicitly classified as source evidence before completion.
...
- Confirm the jargon audit covers the entire delivered Markdown, including captions, tables, diagram labels, headings, metadata, checklist-facing status phrases, and cross-reference text. Search for unexplained English or acronym-heavy phrases outside the centralized terminology chapter, then either translate them, add a first-use Chinese explanation, or document why the English form is necessary. Do not treat a glossary, inventory, or the fact that a term is common to reviewers as a substitute for contextual explanation.
