...
在引入论文方法名、模块缩写或公式之前，必须给出至少一个端到端具体场景。按自然叙事写清：

- 谁或什么系统在处理哪类输入/工作负载；
- 现有流程依次做什么，具体在哪一步失败；
- 用户、模型或系统能观察到什么错误、成本或风险；
- 成功方案必须保留什么，并改变什么行为；
- 场景来自论文的 Figure/case/trace/实验，还是“本文为便于理解所做的重构，不是论文逐字案例/实验”。

场景不能只把抽象词换成口语，也不能先假设读者理解论文方法。读完场景后，未读论文的读者应能独立回答“现在究竟哪里出错，这篇论文必须让什么变得不同？”
...
这部分必须在正文、图表、清单和 manifest 全部冻结后填写，不能用 schema、正则或“文件存在”替代实际检查。
