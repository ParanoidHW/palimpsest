...
- [pending] W4 Paper-level problem-solution rationale: before method acronyms/equations, write an evidence-bounded end-to-end scenario naming the actor/object, input/context, current workflow, exact failure step, visible consequence, and behavior to preserve/change; then cover the background trigger, prior-method root cause, target problem and success criteria, core solution steps, changed variables/system behavior, expected optimization, measured evidence, and remaining boundary; classify the scenario and claims as paper-provided, reviewer-reconstructed, author-stated, inferred, not-stated, or unavailable.
