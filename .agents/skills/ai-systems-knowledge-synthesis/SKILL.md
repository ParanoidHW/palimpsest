...
Apply two composition budgets before adding detail: the mechanism's occupied content should be visually centered within the core frame (unless an intentional asymmetric inset is declared), and each panel should use at most one heading, one subtitle, and two short explanatory notes. Put definitions and caveats in the caption/footer or companion prose; do not fill unused space with paragraph-like labels.
...
Use this execution protocol for every diagram review: (1) the author renders the current source and prepares the full frame plus all named crops; (2) the author delegates a QA-only pass to an independent subagent and sends the exact current paths, diagram contract, and review checklist; (3) the reviewer returns severity-ordered findings and does not edit source files; (4) the author fixes every blocking or visual finding, regenerates the full frame and all affected crops, and sends the new artifacts back to the same reviewer; (5) only an explicit `passed` response for the exact latest render may close the gate. If no independent subagent is available, leave the review status `pending` and report the blocker; never downgrade to author self-approval.
...
The visual QA gate must also inspect composition: compare the bounding box of the primary content to the core frame, flag large one-sided empty regions, check that peer panels or lanes share a visual baseline, and verify that secondary text does not visually outweigh the primary chain. A diagram fails when the content is technically readable but its center of gravity, whitespace, or text density makes the intended hierarchy unclear.
