...
For iterative edits, declare `--scope delta` with `--change-class text|crop`, `--affected-region`, and optional `--neighbor-region`. A delta request must reference the latest passed `full` request (or the script selects it), and its reviewer must report only the affected and neighboring regions. Changes classified as `geometry` or `semantic`, or changes to source/contract hashes relative to the baseline, automatically escalate to `full`. The first request, layout/arrow/font/canvas/legend changes, and publication always require `full`; run `verify --required-scope full` as the release gate. Reviewer claims carry a ten-minute lease; use `heartbeat`/`renew` while inspecting and `reclaim` after expiry. Reclaim never overrides an active lease.
